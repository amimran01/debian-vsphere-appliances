...
"""
    ...
    and https://github.com/tsugliani/packer-vsphere-debian-appliances
"""
...
def run_remote_script(properties):
    """
        Run remote script hosted on a web server
    """
    if properties['guestinfo.remotescripturl']:
        run_cmd = """curl -o /tmp/remote_script -s '{remotescripturl}' && chmod +x /tmp/remote_script && /tmp/remote_script ; rm /tmp/remote_script""".format(remotescripturl=properties['guestinfo.remotescripturl'])
        subprocess.Popen(run_cmd, shell=True, stdout=subprocess.PIPE).stdout.read() 

def run_user_data(properties):
    """
        Run user data
    """
    if properties['guestinfo.userdata']:
        run_cmd = """echo -n '{userdata}' | base64 -d > /tmp/userdata_script && chmod +x /tmp/userdata_script && /tmp/userdata_script ; rm /tmp/userdata_script""".format(userdata=properties['guestinfo.userdata'])
        subprocess.Popen(run_cmd, shell=True, stdout=subprocess.PIPE).stdout.read() 
...
run_remote_script(properties)
run_user_data(properties)