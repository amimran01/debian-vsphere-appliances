...
def appliance_install_kubernetes(properties):
    if properties['guestinfo.kubernetesrelease']:
        kubernetesinstall_cmd = """
# Install kubernetes
apt update
apt install -y kubelet={kubernetesrelease}-00 kubectl={kubernetesrelease}-00 kubeadm={kubernetesrelease}-00
sudo apt-mark hold kubelet kubeadm kubectl
    """.format(
            kubernetesrelease=properties['guestinfo.kubernetesrelease']
        )
        subprocess.Popen(kubernetesinstall_cmd, shell=True, stdout=subprocess.PIPE).stdout.read()
    else: #If kubernetesrelease not set, install the latest version
        kubernetesinstall_cmd = """
# Install kubernetes
...
appliance_install_kubernetes(properties)